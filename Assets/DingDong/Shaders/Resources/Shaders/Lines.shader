	...
	SubShader {
   		...
   		Pass {
		    ...
		    half4 frag (v2f i) : COLOR
		    {
			    ...
			    float lineDistance = 4.0;
			    float lineThickness = 0.1;
			    float lumDisplacementScale = 0.1;
			    ...
			    float displacementTempo = cnoise(float2(uv.x * 10.0 + uv.x * 200.0 * _SamplesTotal, index));

			    y += displacementLum + displacementTempo;
