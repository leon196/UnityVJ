Shader "LargeHadronCollider/Explosion" {
  Properties {
    ...
    SubShader {
      ...
      Pass {
        ...
        void geom(triangle GS_INPUT tri[3], inout TriangleStream<FS_INPUT> triStream)
        {
          ...
          float distorsion = abs(snoise(g * 4.0)) * 2.0;
          a *= lerp(1.0, 1.0 + distorsion, _Offset);
          b *= lerp(1.0, 1.0 + distorsion, _Offset);
          c *= lerp(1.0, 1.0 + distorsion, _Offset);
          /*a += lerp(0.0, 100.0, t);*/
          /*a += 10.0 * tri[0].normal;*/
