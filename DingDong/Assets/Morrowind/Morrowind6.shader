    ...
    SubShader {
      ...
      Pass {
        ...
        void geom(triangle GS_INPUT tri[3], inout TriangleStream<FS_INPUT> triStream)
        {
          ...
          b += triNormal * fft;// * 1.0;*/
          a += tri[0].normal * tex2Dlod(_TextureFFT, float4(vec3ToFloatWay2(tri[0].normal), 0, 0, 0)).r * lum * 0.25;
          b += tri[1].normal * tex2Dlod(_TextureFFT, float4(vec3ToFloatWay2(tri[1].normal), 0, 0, 0)).r * lum * 0.25;
          c += tri[2].normal * tex2Dlod(_TextureFFT, float4(vec3ToFloatWay2(tri[2].normal), 0, 0, 0)).r * lum * 0.25;
