    ...
    SubShader {
      ...
      Pass {
        ...
        void geom(triangle GS_INPUT tri[3], inout TriangleStream<FS_INPUT> triStream)
        {
          ...
          float ttt = fmod(_GlobalFFTTotal * 0.001 + _Offset + x, 1.0);

          float fft = tex2Dlod(_TextureFFT, float4(x, 0, 0, 0)).r;

          float tttt = smoothstep(0.25, 1.0, ttt);
          a = lerp(a, g, tttt);
          b = lerp(b, g, tttt);
          c = lerp(c, g, tttt);
          a += triNormal * ttt + triNormal * (fft + 0.1);
          b += triNormal * ttt + triNormal * (fft + 0.1);
          c += triNormal * ttt + triNormal * (fft + 0.1);
