    ...
    SubShader {
      ...
      Pass {
        ...
        void geom(triangle GS_INPUT tri[3], inout TriangleStream<FS_INPUT> triStream)
        {
          ...
          float x = vec3ToFloatWay2(triNormal);
          x = pingPong(x * 4.0);
          ...
          b += triNormal * fft;
          c += triNormal * fft;*/
          /*a += normalize(a) * tex2Dlod(_TextureFFT, float4(vec3ToFloatWay2(a), 0.0, 0.0, 0.0)).r;
          b += normalize(b) * tex2Dlod(_TextureFFT, float4(vec3ToFloatWay2(b), 0.0, 0.0, 0.0)).r;
          c += normalize(c) * tex2Dlod(_TextureFFT, float4(vec3ToFloatWay2(c), 0.0, 0.0, 0.0)).r;*/
          a += normalize(a) * fft;
          /*b += normalize(b) * fft;*/
          /*c += normalize(c) * fft;*/
        ...
        half4 frag (FS_INPUT i) : COLOR
        {
          ...
          color.rgb = lerp(color.rgb, i.color.rgb, 0.5);
          return color;
        }
        ENDCG
      }
