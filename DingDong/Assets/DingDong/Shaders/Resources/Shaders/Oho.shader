	...
	SubShader {
   		...
   		Pass {
		    ...
		    half4 frag (v2f i) : COLOR
		    {
		    	...
		    	float2 direction = float2(0, 1.0);
		    	float2 uvSampler = uv * 2.0;
		    	// float samplerOnX = tex2D(_SamplerSound, uvSampler).r;
		    	float h = cnoise(float2(lumBuffer, lumBuffer));//length(p.yy) * 10.0));
		    	//float2 offset = direction * 0.1 * _SamplesTotal * dist * lumBuffer;// * sqrt(dist) * clamp(lum * 4.0, 0.0, 1.0);
		    	float2 offset = direction * 0.01 * h;
    			...
    			half4 color = lerp(renderTarget, video, step(0.5, length(video - renderTarget)));
