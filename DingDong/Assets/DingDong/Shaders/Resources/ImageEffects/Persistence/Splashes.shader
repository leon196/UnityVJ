	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float2 offset = float2(cos(angle) * dist, sin(angle) * dist) * 0.005 * fftAcceleration;
				
				float rnd = rand(uv) * PI2;
				offset += float2(cos(rnd), sin(rnd)) * 0.001 * fftAcceleration;
				...
				// float treshold = easeInOutSine(_GlobalFFT, 0.0, 1.0, 0.5);
